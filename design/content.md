# NanoRaider - Content Design

## Content Design Philosophy

**Content serves evolution discovery.** Every activity, zone, dungeon, and raid exists primarily to:
1. **Track personality traits** - Your choices reveal who your hero is becoming
2. **Enable evolution unlocks** - Different paths unlock different evolutions
3. **Create collection moments** - Death reveals which evolution you unlocked, Pokédex-style

Content is NOT just about gear progression - it's about discovering your playstyle through death and collecting evolutions across multiple runs. Each activity contributes to hidden personality stats (Aggression, Cunning, Wisdom, Greed, etc.) that determine which evolution you unlock when you die.

**The core loop is:** Play → Die → Discover Evolution → Learn Prerequisites → Plan Next Run → Collect Them All

---

## Overview

Content is structured in a vertical progression: Zones → Dungeons → Raids

Players move through leveling zones (1-10, 11-20, 21-30), then farm dungeons for Pre-BiS gear, then attempt raid bosses for BiS gear. **But more importantly, every choice you make influences which evolution you'll unlock on death.**

---

## Zones (Leveling Content)

### Zone Structure

Each zone has:
- Level range
- Quest chains (provide XP, gold, gear, story)
- Zone reputation
- Travel time from other zones
- Unique flavor/aesthetic

### Zone Roster

#### **1. Greenhollow (Levels 1-10)**
- **Theme:** Peaceful starter village, pastoral fantasy
- **Activities:** Basic quests, combat training, profession unlocks
- **Travel Time:** N/A (starting zone)
- **Key NPCs:** Mentor NPCs that influence early class choices
- **Loot:** Gray/Green starter gear
- **Evolution Influence:** Balanced (no strong personality traits yet)

#### **2. Stonepeak Mountains (Levels 8-15)**
- **Theme:** Dwarven mining town, rugged terrain
- **Activities:** Mining nodes, dungeon introduction (Irondeep Mines)
- **Travel Time:** 1h from Greenhollow
- **Reputation Reward:** Discount on mining profession
- **Loot:** Green gear with Strength/Stamina focus
- **Evolution Influence:** Good for Master Crafter (crafting focus) or Berserker (combat focus)

#### **3. Twilight Forest (Levels 12-18)**
- **Theme:** Enchanted forest, mystical creatures
- **Activities:** Herbalism nodes, magic-focused quests
- **Travel Time:** 2h from Greenhollow, 1.5h from Stonepeak
- **Reputation Reward:** Alchemy recipes
- **Loot:** Green gear with Intelligence/Crit focus
- **Evolution Influence:** Good for Scholar (wisdom focus) or Alchemist (crafting + herbalism)

#### **4. Dustshore Bay (Levels 16-22)**
- **Theme:** Coastal port town, pirate conflicts
- **Activities:** Gold farming (merchant quests), trade goods
- **Travel Time:** 2h from Twilight Forest
- **Reputation Reward:** Mount discount (400g instead of 500g)
- **Loot:** Green gear with Agility focus
- **Evolution Influence:** Good for Merchant (gold focus) or Treasure Hunter (rare drops)

#### **5. Ashfall Wastes (Levels 20-26)**
- **Theme:** Volcanic badlands, dangerous creatures
- **Activities:** Elite quests, first dungeons (Molten Caverns)
- **Travel Time:** 3h from Dustshore Bay
- **Reputation Reward:** Fire resistance gear (useful for raids)
- **Loot:** Green/Blue gear, crafting materials
- **Evolution Influence:** Good for Berserker (high risk combat) or Survivor (dangerous zones)

#### **6. Shadowspire Citadel (Levels 25-30)**
- **Theme:** Dark fortress, undead/demonic enemies
- **Activities:** Max-level quests, dungeon hub (3 dungeons nearby)
- **Travel Time:** 2h from Ashfall Wastes
- **Reputation Reward:** Access to raid attunement quests
- **Loot:** Blue Pre-BiS gear from quests
- **Evolution Influence:** Good for Scholar (study focus) or Speed Runner (quest efficiency)

---

## Dungeons (Pre-BiS Farming)

Dungeons are the core mid-game loop. Each run takes 2 in-game hours, costs **15 energy**, and has a 30% loot chance per item.

### Dungeon Design Principles

1. **Targeted Loot Tables** - Each dungeon drops specific slots
2. **Multiple Runs Expected** - 30% drop rate = avg 3-4 runs per item
3. **Travel Time Matters** - Dungeons in same zone = more efficient
4. **Difficulty Scaling** - Later dungeons = higher death risk but better loot
5. **Energy Cost** - Each dungeon run costs 15 energy (expensive, but rewarding)

### Dungeon Roster (8 Total)

#### **1. Irondeep Mines** (Levels 15-18)
- **Location:** Stonepeak Mountains
- **Theme:** Abandoned mine, earth elementals
- **Time/Energy:** 2h in-game, 15 energy
- **Difficulty:** Easy (5% death risk with green gear)
- **Loot Table:**
  - Helmet (Blue) - 30%
  - Boots (Blue) - 30%
  - Weapon (Blue, Strength) - 25%
  - Mining materials - 100%
- **Strategic Value:** Early Pre-BiS, safe farming
- **Evolution Influence:** Master Crafter (crafting materials) or Safe Player (low risk content)

#### **2. Whispering Crypts** (Levels 18-22)
- **Location:** Twilight Forest
- **Theme:** Ancient tomb, undead enemies
- **Time/Energy:** 2h in-game, 15 energy
- **Difficulty:** Easy (7% death risk)
- **Loot Table:**
  - Chest (Blue) - 30%
  - Gloves (Blue) - 30%
  - Accessory (Blue, Intelligence) - 25%
  - Dark essences (Enchanting mat) - 100%
- **Strategic Value:** Caster gear focus
- **Evolution Influence:** Scholar (intelligence gear) or Completionist (specific slot farming)

#### **3. Corsair's Den** (Levels 20-24)
- **Location:** Dustshore Bay
- **Theme:** Pirate hideout, naval combat
- **Time/Energy:** 2h in-game, 15 energy
- **Difficulty:** Medium (10% death risk)
- **Loot Table:**
  - Legs (Blue) - 30%
  - Off-hand (Blue) - 30%
  - Weapon (Blue, Agility) - 25%
  - Gold bonus (+20g per run)
- **Strategic Value:** Agility classes, gold farming hybrid
- **Evolution Influence:** Merchant (gold bonus) or Treasure Hunter (rare drops)

#### **4. Molten Caverns** (Levels 22-26)
- **Location:** Ashfall Wastes
- **Theme:** Lava caves, fire elementals
- **Time/Energy:** 2h in-game, 15 energy
- **Difficulty:** Medium (12% death risk)
- **Loot Table:**
  - Helmet (Blue) - 30%
  - Chest (Blue) - 30%
  - Boots (Blue) - 30%
  - Fire resistance gear - 20%
- **Strategic Value:** Multiple body slots, raid prep (fire res)
- **Evolution Influence:** Raid Prepper (resistance gear) or Berserker (medium-risk combat)

#### **5. Scholomance** (Levels 24-28)
- **Location:** Shadowspire Citadel (1h travel)
- **Theme:** Dark academy, necromancers
- **Time/Energy:** 2h in-game, 15 energy
- **Difficulty:** Medium-High (15% death risk)
- **Loot Table:**
  - Helmet (Blue) - 35%
  - Gloves (Blue) - 35%
  - Weapon (Blue, Intelligence) - 30%
  - Dark Runes (Crafting mat) - 100%
- **Strategic Value:** High drop rates, caster focus, THE Pre-BiS farming dungeon
- **Evolution Influence:** Scholar (study focus) or Treasure Hunter (rare drops)

#### **6. Stratholme** (Levels 26-30)
- **Location:** Shadowspire Citadel (1h travel)
- **Theme:** Plagued city, undead scourge
- **Time/Energy:** 2h in-game, 15 energy
- **Difficulty:** High (18% death risk)
- **Loot Table:**
  - Chest (Blue) - 35%
  - Legs (Blue) - 35%
  - Accessory (Blue) - 30%
  - Holy water (Consumable) - 50%
- **Strategic Value:** Best body armor pieces
- **Evolution Influence:** Risk Taker (high death risk) or Optimizer (targeted farming)

#### **7. Temple of the Serpent** (Levels 26-30)
- **Location:** Ashfall Wastes (2h travel)
- **Theme:** Ancient temple, snake cultists
- **Time/Energy:** 2h in-game, 15 energy
- **Difficulty:** High (20% death risk)
- **Loot Table:**
  - Weapon (Blue, all types) - 40%
  - Off-hand (Blue) - 35%
  - Accessory (Blue) - 30%
- **Strategic Value:** Weapon specialists, hybrid classes
- **Evolution Influence:** Weapon Master (weapon focus) or Risk Taker (high death risk)

#### **8. Blackrock Depths** (Levels 28-30)
- **Location:** Ashfall Wastes (2h travel)
- **Theme:** Underground fortress, dark iron dwarves
- **Time/Energy:** 2h in-game, 15 energy
- **Difficulty:** Very High (25% death risk)
- **Loot Table:**
  - All slots (Blue) - 25% each
  - Gold bonus (+40g)
  - Blacksmithing recipes - 30%
- **Strategic Value:** Jack-of-all-trades, high risk/reward
- **Evolution Influence:** Berserker (highest risk combat) or Master Crafter (recipes)

---

## Raids (BiS Endgame)

Raids are the ultimate challenge. **6h in-game time, 30 energy cost**, weekly lockout, high death risk, best loot.

### Raid Design Principles

1. **Weekly Lockouts** - Attempt once per 7 in-game days (simulated reset timer)
2. **Boss Knowledge Matters** - Study system directly impacts survival
3. **BiS Gear Drops** - Purple items, huge power spike
4. **Progressive Difficulty** - Later bosses = harder but better loot
5. **Story Payoff** - Epic moments, cinematic deaths
6. **High Energy Cost** - 30 energy per raid attempt (new players can only do 1 raid with full energy bar)

### Raid Structure: **The Eternal Sanctum**

5-boss mega-raid (inspired by WoW raid tiers)

#### **Boss 1: Molten Fury** (Fire Elemental Lord)
- **Time/Energy:** 6h in-game, 30 energy
- **Difficulty:** Moderate (Base 40% death risk)
- **Required Prep:** Fire resistance gear (from Molten Caverns), solid blue gear
- **Loot Table:**
  - Helmet (Purple) - 50%
  - Boots (Purple) - 50%
- **Boss Knowledge Cap:** 50% (max mitigation from study)
- **Thematic:** Guardian of the entrance, tests basic competence
- **Lockout:** 7 in-game days
- **Evolution Influence:** Raid Prepper (resistance gear) or Scholar (study focus)

#### **Boss 2: Shadowmaw** (Giant Undead Beast)
- **Time/Energy:** 6h in-game, 30 energy
- **Difficulty:** High (Base 50% death risk)
- **Required Prep:** High Stamina, healing consumables
- **Loot Table:**
  - Chest (Purple) - 50%
  - Gloves (Purple) - 50%
- **Boss Knowledge Cap:** 60%
- **Mechanic:** Enrage timer (DPS race, needs good weapon)
- **Lockout:** 7 in-game days
- **Evolution Influence:** Berserker (DPS race) or Optimizer (consumable efficiency)

#### **Boss 3: Lady Veilstrike** (Assassin Queen)
- **Time/Energy:** 6h in-game, 30 energy
- **Difficulty:** High (Base 55% death risk)
- **Required Prep:** High armor, defensive consumables
- **Loot Table:**
  - Legs (Purple) - 50%
  - Accessory (Purple) - 50%
- **Boss Knowledge Cap:** 65%
- **Mechanic:** Burst damage spikes (requires studying her patterns)
- **Lockout:** 7 in-game days
- **Evolution Influence:** Scholar (pattern study) or Survivor (defensive play)

#### **Boss 4: Archmagus Pyraxis** (Elemental Wizard)
- **Time/Energy:** 6h in-game, 30 energy
- **Difficulty:** Very High (Base 65% death risk)
- **Required Prep:** Resistance gear, high DPS
- **Loot Table:**
  - Weapon (Purple) - 50%
  - Off-hand (Purple) - 50%
- **Boss Knowledge Cap:** 75%
- **Mechanic:** Complex spell rotations (highest knowledge requirement)
- **Lockout:** 7 in-game days
- **Evolution Influence:** Scholar (max study) or Perfectionist (complex mechanics)

#### **Boss 5: Zar'thul the Eternal** (Final Boss)
- **Time/Energy:** 6h in-game, 30 energy
- **Difficulty:** Extreme (Base 75% death risk)
- **Required Prep:** Full BiS from previous bosses, max consumables, max knowledge
- **Loot Table:**
  - Any slot (Purple) - 60%
  - Legendary accessory - 10% (massive power spike)
- **Boss Knowledge Cap:** 85%
- **Mechanic:** All-or-nothing encounter, "final exam" of character run
- **Lockout:** 7 in-game days
- **Story:** Defeating = "Raid Legend" achievement, glory ending
- **Evolution Influence:** Raid Legend (requires Berserker + Scholar prerequisite evolutions)

### Raid Lockout System

- Attempt any boss = locks THAT boss for 7 in-game days
- Can attempt other bosses during same in-game week
- Die to Boss 1 on In-Game Day 10? Can't retry until In-Game Day 17
- Creates strategic tension: "Am I ready or should I farm more?"
- With aging kicking in at Day 13+, late raid attempts have high stakes

**Optional: Heroic Mode**
- After defeating normal mode, unlock Heroic (same boss, +15% death risk)
- Drops Legendary gear (orange)
- For true masochists and achievement hunters

---

## Boss Knowledge System (Study Content)

Each raid boss has study activities (**2h in-game, 12 energy each**, diminishing returns).

### Study Activities (Example: Molten Fury)

1. **Read Combat Logs** (+5% knowledge, 2h, 12 energy) - "Analyze other players' strategies"
2. **Watch Strat Videos** (+5% knowledge, 2h, 12 energy) - "Study the boss mechanics"
3. **Practice Rotation** (+4% knowledge, 2h, 12 energy) - "Simulate your DPS rotation"
4. **Gather Intel** (+3% knowledge, 2h, 12 energy) - "Talk to guild members who've faced him"
5. **Meditation** (+2% knowledge, 2h, 12 energy) - "Mentally prepare for the fight"

**Study Investment:**
- After 5 study sessions = 19% knowledge, 60 energy invested, 10h in-game time
- Still need gear + consumables to survive 40% base risk
- Studying same boss 10+ times = 50% knowledge (cap), 120 energy invested
- Now need 0 additional mitigation to have ~10% death risk

**Energy Trade-off:**
- New players (50 energy): Can do 4 study sessions OR 3 dungeon runs per full energy bar
- Veterans (200 energy): Can do 16 study sessions OR 13 dungeon runs per session
- Study vs farm is a strategic choice

---

## Evolution-Specific Content

**Not all content matters for all evolutions.** This is a KEY design principle - your choices reveal your playstyle.

### Evolution Examples and What Matters

**Berserker (Tier 1)**
- **What Matters:** Combat time, high-risk dungeons (Blackrock Depths, Temple of Serpent), raid boss attempts
- **What Doesn't:** Crafting, gold farming, low-risk safe content
- **Efficiency Note:** Berserker needs combat time, so spending 15 energy on dangerous dungeons is MORE efficient for evolution progress than safe quests

**Merchant (Tier 1)**
- **What Matters:** Gold earned, gold farming activities, trading, selling loot instead of using it
- **What Doesn't:** Combat efficiency, study time, gear optimization
- **Efficiency Note:** Merchant needs gold accumulation time, so Corsair's Den (+20g bonus) is better than Scholomance

**Scholar (Tier 1)**
- **What Matters:** Study time, boss knowledge sessions, reading lore, intelligence-focused content
- **What Doesn't:** Combat speed, gold farming, crafting
- **Efficiency Note:** Scholar needs study time (12 energy per session), so prioritize boss knowledge over dungeon spam

**Master Crafter (Tier 1)**
- **What Matters:** Crafting time, gathering materials, profession activities, crafting recipes
- **What Doesn't:** Combat efficiency, raid attempts, gold hoarding
- **Efficiency Note:** Crafting activities take time - 2h to craft consumables counts toward evolution even if it costs 8 energy

**Raid Legend (Tier 2)**
- **What Matters:** Defeating Zar'thul (final boss), having both Berserker + Scholar prerequisite evolutions unlocked
- **What Doesn't:** Speed of victory, gear quality, gold remaining
- **Prerequisite System:** You MUST unlock Berserker and Scholar in previous runs before Raid Legend becomes discoverable

### How Different Evolutions Change Energy Efficiency

**Same 15-energy dungeon run, different evolution goals:**

| Evolution Target | Efficient Content | Inefficient Content |
|-----------------|-------------------|---------------------|
| Berserker | Blackrock Depths (25% death risk = combat time) | Irondeep Mines (5% death risk = too safe) |
| Scholar | Study sessions (12 energy, 2h study time) | Gold farming (8 energy, but no wisdom gain) |
| Merchant | Corsair's Den (15 energy + 20g bonus) | Whispering Crypts (no gold bonus) |
| Master Crafter | Gathering materials + crafting (8-12 energy) | Speed running quests (10 energy but no crafting) |
| Treasure Hunter | High-loot dungeons with rare drops | Safe quests with guaranteed green loot |

**This creates strategic depth:** "Should I farm Scholomance for Scholar evolution (high study requirements) or rush combat for Berserker evolution (high risk content)?"

### Evolution Discovery on Death

When you die, the game reveals:
1. **Which evolution you unlocked** (big celebration, Pokédex-style reveal)
2. **Why you unlocked it** (e.g., "You spent 40% of your time in combat and took high risks - you've become a **Berserker**!")
3. **What bonuses it gives** (Knowledge Transfer - future runs start with Berserker's +10% damage)
4. **What prerequisite it is for** (e.g., "Berserker is a prerequisite for Raid Legend, Gladiator, and Weapon Master")
5. **What you DIDN'T unlock** (Hint system - "You were close to Merchant evolution! Try earning more gold next run.")

---

## Loot Table Summary

### Full Gear Progression Path (Example: Helmet Slot)

| Rarity | Item Name | Source | Item Power | Notes |
|--------|-----------|--------|------------|-------|
| Gray | Worn Helmet | Starting gear | 10 | Vendor trash |
| Green | Iron Helm | Quest reward (Stonepeak) | 25 | Early upgrade |
| Blue | Shadowforged Helm | Scholomance (35% drop) | 50 | Pre-BiS |
| Purple | Eternal Helm | Molten Fury raid boss (50% drop) | 100 | BiS |
| Orange | Crown of Legends | Zar'thul (10% drop) | 150 | Dream item |

### Strategic Farming Paths

**Evolution-focused paths replace traditional min-maxing:**

**Berserker Evolution Path:**
- Prioritize high-risk combat (Blackrock Depths, Temple of Serpent)
- Skip safe content (Irondeep Mines, low-level quests)
- Attempt raid bosses with minimal prep (high death risk = more combat time)
- Goal: Die in glorious combat after maximizing combat time
- Energy efficiency: Combat dungeons (15 energy) > Study sessions

**Scholar Evolution Path:**
- Prioritize study sessions (12 energy each, 2h study time)
- Max out boss knowledge for all raid bosses
- Farm intelligence-focused content (Whispering Crypts, Scholomance)
- Skip gold farming and crafting
- Goal: Die with maximum knowledge accumulated
- Energy efficiency: Study sessions > Intelligence dungeons > Combat

**Merchant Evolution Path:**
- Prioritize gold farming activities (Corsair's Den +20g, Blackrock Depths +40g)
- Buy and sell items frequently (trade activity)
- Skip gear optimization (sell blues instead of equipping them)
- Hoard gold until death
- Goal: Die with maximum gold accumulated
- Energy efficiency: Gold-bonus dungeons > Regular dungeons

**Master Crafter Evolution Path:**
- Prioritize crafting and gathering (8-12 energy, 2-4h time investment)
- Farm materials from dungeons (Mining materials, Dark Runes, etc.)
- Spend time crafting consumables even if you don't need them
- Collect all crafting recipes
- Goal: Die with maximum crafting time logged
- Energy efficiency: Crafting time > Material farming > Combat

**Raid Legend Evolution Path (Tier 2):**
- Requires: Must have unlocked Berserker + Scholar in previous runs first
- Strategy: Combine combat focus with study focus
- Defeat Zar'thul (final boss)
- This evolution is IMPOSSIBLE on your first run (prerequisite system)
- Goal: Die after defeating final boss while having correct prerequisites
- Energy efficiency: Balance combat and study equally

**Traditional "Optimal" Path (Still Valid):**
- Focus on worst 3-4 gear slots (biggest upgrades)
- Study each boss 5-7 times (diminishing returns)
- Use gold for consumables instead of mount (short-term power)
- Balance risk and speed
- Note: This path might unlock a "Optimizer" or "Perfectionist" evolution

---

## Random Events Content

### Event Types Examples

**Trade Events:**
- "Rare vendor selling [Purple Weapon] for 800g!" (Opportunity cost: all your gold)
- "Player selling [Blue Helmet] for 100g in trade chat" (Skip farming that dungeon)

**Guild Events:**
- "Guild needs DPS for Molten Fury tonight!" (Join raid early, higher death risk)
- "Guild offering free consumables to new raiders" (Save gold, lose time traveling)

**World Events:**
- "Server-wide XP boost today!" (Optimize questing vs dungeon farming)
- "AH prices crashed, consumables 50% off!" (Stock up now or miss opportunity)

**Crisis Events:**
- "Rare spawn boss in Ashfall Wastes!" (Drop guaranteed blue, but 3h detour)
- "Your faction is losing PvP battle!" (Participate for honor rewards or ignore)

---

## Content Expansion Hooks (Future)

### Additional Raids
- **Dragon's Lair** - 3-boss raid, dragon-themed loot
- **Nightmare Realm** - Dream-based encounters, psychological horror theme

### PvP Content
- Arena seasons (compete for rankings)
- Battlegrounds (team-based objectives)

### Seasonal Events
- Holiday events (special cosmetics, time-limited achievements)
- World bosses (server-wide cooperation)

### New Classes (Meta Unlocks)
- Necromancer (raise minions, high complexity)
- Bard (support/hybrid, versatile playstyle)
- Demon Hunter (high risk, high reward DPS)

---

## Content Balancing Principles

1. **Multiple paths to same goal** - Different dungeons can drop similar slots
2. **No dead content** - Early dungeons still useful for alts or specific items
3. **RNG but targetable** - Drop rates are fair, farming is strategic not pure luck
4. **Time investment respects player** - 2h dungeon should feel meaningful
5. **Endgame is aspirational** - Raids are hard but achievable with prep

---

## Content Pacing (Typical Run)

**Evolution discovery changes how you think about pacing:**

**First Run (Discovery Mode):**
- You don't know what evolutions exist yet
- Play naturally, die, discover your first evolution
- Death reveals: "You unlocked **Berserker** evolution!"
- Collection screen unlocks (Pokédex UI)
- Real-time: ~2-4 weeks (50 energy capacity)

**Second Run (Targeted Evolution):**
- Goal: Unlock a DIFFERENT evolution (collect them all!)
- Maybe you unlocked Berserker first, now try for Scholar
- Intentionally change playstyle (more study time, less combat)
- Die, discover Scholar evolution
- Collection screen shows: 2/30 evolutions unlocked
- Real-time: ~1-2 weeks (100 energy capacity after first death)

**Third+ Runs (Prerequisite Hunting):**
- You've seen hints: "Raid Legend requires Berserker + Scholar"
- Goal: Unlock Tier 2 evolution by having prerequisites
- Combine combat and study, defeat Zar'thul
- Death reveals: "You unlocked **Raid Legend** evolution! (Tier 2)"
- Collection screen shows: 3/30 evolutions unlocked
- Real-time: ~1 week (150+ energy capacity)

**Traditional Pacing (Still Relevant):**

**In-Game Days 1-5 (Leveling):**
- Zones: Greenhollow → Stonepeak → Twilight → Dustshore → Ashfall → Shadowspire
- Activities: 90% questing (10 energy), 10% exploration
- Evolution tracking: Hidden personality stats building (Aggression, Wisdom, Greed, etc.)

**In-Game Days 6-10 (Pre-BiS Farming):**
- Dungeons: Choice based on evolution target
  - Berserker path: Blackrock Depths (high risk)
  - Scholar path: Study sessions + intelligence dungeons
  - Merchant path: Corsair's Den, Blackrock Depths (gold bonuses)
- Activities: 70% targeted content, 20% support activities, 10% preparation
- Evolution tracking: Playstyle becoming clear

**In-Game Days 11-14 (Raiding):**
- Raids: Attempts based on evolution goal
  - Berserker: Rush bosses with minimal prep (die in combat)
  - Scholar: Max study, attempt with knowledge
  - Raid Legend: Defeat Zar'thul (only possible with prerequisites)
- Activities: 50% raids/study, 30% final evolution push, 20% gear optimization
- Evolution tracking: Final push to meet evolution thresholds

**In-Game Day 15-18+ (Overtime):**
- Aging penalties kick in
- Last chance to shift evolution trajectory
- Death approaching = evolution reveal imminent
- Real-time: ~1-4 real days

**Total Character Run:**
- **18 in-game days** minimum to reach endgame
- **Death reveals which evolution you unlocked** (core moment)
- **Real-time varies:**
  - Casual (50 energy): 2-4 weeks per run
  - Active (100 energy): 1-2 weeks per run
  - Veteran (200+ energy): 3-7 days per run
- **Collection goal:** 30 evolutions total, will take months to unlock all
